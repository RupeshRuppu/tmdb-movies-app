...
const generateSeats = () => {
  const numRows = 7; // odd number ensures symmetry
  const maxColumns = 9;
  let rowArray = [];
  let start = 1;

  // Define seat count per row (hardcoded for full control)
  const seatPattern = [3, 5, 7, 9, 7, 5, 3];

  for (let i = 0; i < numRows; i++) {
    let numColumns = seatPattern[i];
    let columnArray = [];
    ...
    <ScrollView style={styles.container}>
      ...
      <View>
        <ImageBackground
          ...
          source={{
            uri: baseImagePath('w342', route.params.BgImage),
          }}>
          <LinearGradient
